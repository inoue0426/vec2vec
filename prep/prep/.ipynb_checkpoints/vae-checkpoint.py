...
class VAE(BaseAE):
    ...
    # --- in VAE.loss_function ---
    def loss_function(self, *args, **kwargs):
        x, recons, mu, log_var = args[:4]
    
        recons_loss = F.mse_loss(x, recons)
    
        # clamp log_var to prevent variance blow-up
        log_var = log_var.clamp(min=-6.0, max=2.0)
    
        kl_per_dim = -0.5 * (1 + log_var - mu.pow(2) - log_var.exp())
        kl = kl_per_dim.sum(dim=1).mean()
    
        beta  = kwargs.get("beta", 1.0)
        C     = kwargs.get("capacity", 8.0)
        margin= kwargs.get("margin", 2.0)
    
        C_low, C_high = C - margin, C + margin
        cap_penalty = F.relu(C_low - kl) + F.relu(kl - C_high)
    
        loss = recons_loss + beta * cap_penalty
        return {"loss": loss, "recons_loss": recons_loss, "KLD": kl}
